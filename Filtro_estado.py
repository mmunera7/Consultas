...
class DataBase:
    ...
    def select_propertys(self, id):
        sql = "SELECT * FROM status_history JOIN property ON status_history.property_id = property.id JOIN status ON status_history.status_id = status.id WHERE status.name ='{}'".format(id)
